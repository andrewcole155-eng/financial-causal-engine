...
# ==============================================================================
# --- NEW TAB: HEDGE FUND DASHBOARD (Price + Risk) ---
# ==============================================================================
def render_forecast_dashboard():
    """
    Reads the latest 'live_forecasts.csv' generated by predict.py 
    and displays a Hedge Fund style dashboard.
    """
    st.header("📈 AI Forecast Dashboard")
    st.caption("Temporal GNN Predictions for the Next Trading Day.")
    ...
    try:
        ...
        if not os.path.exists(csv_path):
            st.warning("⚠️ No forecast data found. Run 'python predict.py' first.")
            return
    ...
    # 2. KPI Metrics (Top Movers)
    # Sort by Forecast Return
    ...
    with col1:
        st.metric("🚀 Top Long Pick", top_bull['ticker'], f"{top_bull['forecast_return']:.2%}")
    with col2:
        st.metric("📉 Top Short Pick", top_bear['ticker'], f"{top_bear['forecast_return']:.2%}")
    with col3:
        st.metric("⚠️ Market Risk Avg", f"{avg_risk:.2f}", help="0=Safe, 1=Danger")
    with col4:
        st.metric("📊 Assets Analyzed", len(df))

    st.divider()

    # 3. The "Efficient Frontier" Scatter Plot (Risk vs Return)
    # We want High Return (Y-axis Up) and Low Risk (X-axis Left)
    
    # Add Color categorization
    def get_color(row):
        if row['risk_score'] > 0.7: return 'High Risk'
        if row['forecast_return'] > 0.01: return 'High Growth'
        return 'Neutral'
    
    df['Category'] = df.apply(get_color, axis=1)
    ...
    fig = px.scatter(
        ...
        hover_data=["ticker"],
        color="Category",
        color_discrete_map={'High Risk': '#EF553B', 'High Growth': '#00CC96', 'Neutral': '#636EFA'},
        title="Risk/Reward Map (The Alpha Seeker)",
        labels={"risk_score": "AI Predicted Risk (0-1)", "forecast_return": "Predicted Return (%)"}
    )
    ...
    fig.update_layout(
        template="plotly_dark", 
        height=500,
        xaxis=dict(range=[0, 1]), # Risk is 0-1
    )
    
    st.plotly_chart(fig, use_container_width=True)

    # 4. Data Tables
    col_long, col_risk = st.columns(2)
    
    with col_long:
        st.subheader("🟢 Top Bullish Forecasts")
        # Show top 10 positive returns
        bull_df = df_sorted.head(10)[['ticker', 'forecast_return', 'risk_score']]
        # Style formatting
        st.dataframe(
            bull_df.style.format({'forecast_return': '{:.2%}', 'risk_score': '{:.2f}'})
                   .background_gradient(subset=['forecast_return'], cmap='Greens'),
            hide_index=True,
    ...
    with col_risk:
        ...
        risk_df = df.sort_values(by='risk_score', ascending=False).head(10)[['ticker', 'forecast_return', 'risk_score']]
        st.dataframe(
            risk_df.style.format({'forecast_return': '{:.2%}', 'risk_score': '{:.2f}'})
...
def main():
    ...
    # --- UPDATED TABS DEFINITION (Added "📈 Forecasts" as Tab 1) ---
    tab_forecast, tab_events, tab_explore, tab_simulate, tab_path, tab_xray, tab_help = st.tabs([
        "📈 Forecasts", "🔔 Recent Events", "🗺️ Explore Graph", "🔬 Simulate Scenarios", "↔️ Causal Pathfinding", "🔮 GNN X-Ray", "📘 Help & Guide"
    ])

    # --- TAB 1: FORECAST DASHBOARD (NEW) ---
    with tab_forecast:
        render_forecast_dashboard()

    # --- TAB 2: RECENT EVENTS (UPDATED WITH FILTERS) ---
    with tab_events:
    ...
    with tab_simulate:
        ...
        with col_inputs:
            ...
            # --- MODE B: EVOLVED SINGLE ASSET SHOCK ---
                ...
                sec_filter = st.selectbox("Sector:", ["All"] + all_sectors)
        ...
        # ==========================================================================
        if run_btn:
            with st.spinner("Calculating propagation..."):
                aggregated_impacts = {}
                
                if not simulation_inputs:
                    st.warning("No shocks applied.")
                else:
                    # 1. Propagate
        ...
        with col_explain:
            ...
            if st.session_state.sim_results:
                ...
                st.divider()
                if gemini_active and st.button("✨ Generate AI Narrative"):
